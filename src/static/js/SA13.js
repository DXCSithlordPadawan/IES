...
function displayHelp() {
    console.log('🚀 SA-13 Gopher Surface-to-Air Missile System Database Management Tool');
    console.log('==================================================================\n');
    console.log('Usage: node SA13.js [command] [options]\n');
    console.log('Commands:');
    console.log('  --add              Add SA-13 Gopher system to specified database');
    console.log('  --del              Remove SA-13 Gopher system from specified database');
    ...
    console.log('  node SA13.js --add --db OP7     Add SA-13 to Odesa Oblast database');
    console.log('  node SA13.js --del --db OP1     Remove SA-13 from Donetsk Oblast database');
    console.log('  node SA13.js --list             Show all available databases');
    console.log('  node SA13.js --diagnostic       Run system diagnostics');
}

// Function to run comprehensive diagnostics
function runDiagnostics() {
    console.log('🔧 Running SA13.js Diagnostic Checks');
    console.log('===================================\n');
...
// Function to clear any cached data that might contain SA-13
...
// Function to add SA-13 Gopher missile system to the JSON data
function addSA13System() {
    ...
    const systemId = `air-defense-sa13-gopher-${OPERATION_CONFIG.database.toLowerCase()}-001`;
    
    // Create the SA-13 Gopher missile system entry following IES4 schema
    const sa13System = {
        "id": systemId,
        "uri": "https://en.wikipedia.org/wiki/SA-13_Gopher",
        ...
        "names": [
            ...
            {
                "value": "9K35 Strela-10",
                "language": "en",
                "nameType": "grau_designation"
            },
            {
                "value": "9К35 «Стрела-10»",
                "language": "ru",
                "nameType": "official"
            },
            {
        ...
        "identifiers": [
            ...
            {
                "value": "9K35",
                "identifierType": "GRAU_INDEX",
                "issuingAuthority": "Soviet Ministry of Defense"
            },
            {
                "value": "Strela-10",
        ...
        "systemType": "short-range-surface-to-air-missile",
        "manufacturer": "KBM (Kolomna)",
        "model": "9K35 Strela-10",
        "enteredService": 1976,
        "operator": "Various Warsaw Pact and export countries",
        "location": OPERATION_CONFIG.displayName,
        "representations": [
            {
                "id": "sa13-wiki",
                "type": "webpage",
                "title": "SA-13 Gopher - Wikipedia",
                "description": "Wikipedia article about the SA-13 Gopher (9K35 Strela-10) surface-to-air missile system",
                "url": "https://en.wikipedia.org/wiki/SA-13_Gopher"
            }
        ],
        "temporalParts": [
            {
                "id": "sa13-development-start",
            ...
            {
                "id": "sa13-first-test",
                "stateType": "prototype_testing",
                "startTime": "1970-01-01T00:00:00Z",
                "location": "Soviet Union"
            },
            {
                "id": "sa13-service-entry",
                "stateType": "entered_service",
                "startTime": "1976-01-01T00:00:00Z",
                "location": "Soviet Armed Forces"
            },
            {
                "id": `sa13-deployed-${OPERATION_CONFIG.database.toLowerCase()}`,
        ...
        "states": [
            {
                "id": `sa13-operational-${OPERATION_CONFIG.database.toLowerCase()}`,
        ...
        "specifications": {
            "missileType": "9M37 surface-to-air missile",
            "range": "0.8-5.0 km",
            "altitude": "25-3,500 m",
            "guidance": "Infrared homing",
            "warhead": "3 kg fragmentation",
            "launcher": "MT-LB tracked vehicle",
            "crew": 3,
            "missilesPerLauncher": 4,
            "reloadTime": "5 minutes",
            "maxSpeed": "Mach 2.5 (missile)",
            "weight": "42 kg (missile)",
            "length": "2.2 m (missile)",
    ...
    // Create air defense system type definition
    const airDefenseSystemType = {
        "id": "short-range-surface-to-air-missile",
        "name": "Short Range Surface-to-Air Missile System",
        "category": "air defense system",
        "subcategory": "surface-to-air missile",
        "description": "Mobile short-range surface-to-air missile system for low-altitude air defense",
        "characteristics": [
            {
                "name": "primary_role",
                "value": "air defense",
    ...
    // Add air defense systems array if it doesn't exist
    if (!data.airDefenseSystems) {
        data.airDefenseSystems = [];
    }
    
    // Add air defense system types array if it doesn't exist
    if (!data.airDefenseSystemTypes) {
        data.airDefenseSystemTypes = [];
    }
    
    // Check if SA-13 already exists (check for any SA-13 in this database)
    const existingSystem = data.airDefenseSystems.find(s => 
        s.id.includes('air-defense-sa13-gopher') || 
        (s.names && s.names.some(name => 
            name.value === 'SA-13 Gopher' || 
            name.value === '9K35 Strela-10' ||
            name.value === 'Strela-10'
        ))
    );
    
    if (existingSystem) {
        console.log(`ℹ️ SA-13 Gopher system already exists in ${OPERATION_CONFIG.displayName} database. Updating...`);
        const index = data.airDefenseSystems.findIndex(s => s.id === existingSystem.id);
        data.airDefenseSystems[index] = { ...sa13System, id: existingSystem.id };
    } else {
        // Add the system to the air defense systems array
        data.airDefenseSystems.push(sa13System);
    }
    
    // Add the system type if it doesn't already exist
    const existingSystemType = data.airDefenseSystemTypes.find(st => st.id === "short-range-surface-to-air-missile");
    if (!existingSystemType) {
        data.airDefenseSystemTypes.push(airDefenseSystemType);
    }
    
    // Write the updated data back to the file
    fs.writeFileSync(filePath, JSON.stringify(data, null, 2));
    
    console.log(`✅ Successfully added SA-13 Gopher system to ${OPERATION_CONFIG.displayName}`);
    console.log('📁 File location:', filePath);
    console.log('🆔 Added system ID:', sa13System.id);
    console.log('🏷️ System type added:', airDefenseSystemType.id);
    
    return data;
}

// Function to remove SA-13 Gopher system from the JSON data
function removeSA13System() {
    const filePath = path.join('C:', 'ies4-military-database-analysis', 'data', OPERATION_CONFIG.dataFile);
    ...
    try {
        ...
        // Remove SA-13 from air defense systems array
        if (data.airDefenseSystems && Array.isArray(data.airDefenseSystems)) {
            const initialLength = data.airDefenseSystems.length;
            data.airDefenseSystems = data.airDefenseSystems.filter(system => {
                // Check multiple possible identifiers for the SA-13
                const isSA13System = 
                    system.id.includes('air-defense-sa13-gopher') ||
                    (system.names && system.names.some(name => 
                        name.value === 'SA-13 Gopher' || 
                        name.value === '9K35 Strela-10' ||
                        name.value === 'Strela-10' ||
                        name.value === '9К35 «Стрела-10»'
                    )) ||
            ...
            removedCount = initialLength - data.airDefenseSystems.length;
        }
        
        // Check if we should remove the short-range SAM system type
        // (only if no other short-range SAM systems remain)
        if (data.airDefenseSystemTypes && Array.isArray(data.airDefenseSystemTypes)) {
            const hasOtherShortRangeSAMs = data.airDefenseSystems && data.airDefenseSystems.some(system => 
                system.systemType === 'short-range-surface-to-air-missile'
            );
            
            if (!hasOtherShortRangeSAMs) {
                const initialTypeLength = data.airDefenseSystemTypes.length;
                data.airDefenseSystemTypes = data.airDefenseSystemTypes.filter(st => 
                    st.id !== 'short-range-surface-to-air-missile'
                );
                removedSystemType = initialTypeLength > data.airDefenseSystemTypes.length;
            }
        }
        ...
        // Report results
        if (removedCount > 0) {
            console.log(`✅ Successfully removed ${removedCount} SA-13 Gopher system record(s) from ${OPERATION_CONFIG.displayName}`);
            if (removedSystemType) {
                console.log('✅ Also removed short-range-surface-to-air-missile system type (no other short-range SAMs found)');
            }
        } else {
            console.log(`ℹ️ No SA-13 Gopher system records found to remove from ${OPERATION_CONFIG.displayName}`);
        }
        ...
        if (error.code === 'ENOENT') {
            ...
            console.error('❌ Error removing SA-13 Gopher system:', error.message);
        }
        return false;
    }
}
...
function verifyFileStructure() {
    ...
    try {
        ...
        console.log(`   Air Defense Systems: ${data.airDefenseSystems ? data.airDefenseSystems.length : 0}`);
        console.log(`   Air Defense System Types: ${data.airDefenseSystemTypes ? data.airDefenseSystemTypes.length : 0}`);
        
        // Check for any remaining SA-13 references
        let remainingRefs = 0;
        if (data.airDefenseSystems) {
            data.airDefenseSystems.forEach(system => {
                if (system.names && system.names.some(name => 
                    name.value.toLowerCase().includes('sa-13') || 
                    name.value.toLowerCase().includes('gopher') ||
                    name.value.toLowerCase().includes('strela-10')
                )) {
                    remainingRefs++;
                    console.log(`   Found SA-13 reference: ${system.id}`);
                }
            });
        }
        
        console.log(`   SA-13 references found: ${remainingRefs}`);
        
        if (remainingRefs === 0) {
            console.log('✅ No remaining SA-13 Gopher system references found');
        }
...
async function performAddOperation() {
    console.log(`🚀 Adding SA-13 Gopher system to ${OPERATION_CONFIG.displayName} database...\n`);
    ...
    try {
        ...
        if (refreshSuccess) {
            ...
            console.log(`   ✅ SA-13 Gopher system added to ${OPERATION_CONFIG.displayName}`);
...
async function performRemoveOperation() {
    console.log(`🚀 Removing SA-13 Gopher system from ${OPERATION_CONFIG.displayName} database...\n`);
    ...
    try {
        ...
        if (refreshSuccess) {
            ...
            console.log(`   ✅ SA-13 Gopher system removed from ${OPERATION_CONFIG.displayName}`);
            console.log('   ✅ Database reloaded in web interface');
            console.log('   ✅ Analysis refreshed with updated data');
            console.log('   ✅ Cache cleared');
            console.log('\n🌐 You can now view the updated data in your web browser');
            console.log('💡 The SA-13 Gopher system should no longer appear in any analysis');
        } else {
            console.log('\n⚠️ Partial success:');
            console.log(`   ✅ SA-13 Gopher system removed from ${OPERATION_CONFIG.displayName}`);
            console.log('   ✅ Database reloaded in web interface');
...
if (require.main === module) {
    ...
    try {
        ...
        try {
            ...
            console.log('\n💡 Try running: node SA13.js --diagnostic');
            process.exit(1);
        }
