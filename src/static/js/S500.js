...
// Function to clear any cached data that might contain S-500
...
// Function to add S-500 missile system to the JSON data
function addS500MissileSystem() {
    ...
    const systemId = `weapon-system-s500-prometheus-${OPERATION_CONFIG.database.toLowerCase()}-001`;
    
    // Create the S-500 missile system entry following IES4 schema
    const s500MissileSystem = {
        "id": systemId,
        "uri": "https://en.wikipedia.org/wiki/S-500_missile_system",
        "type": "Vehicle",
        "names": [
            {
                "value": "S-500 Prometheus",
                "language": "en",
                "nameType": "official"
            },
            {
                "value": "С-500 Прометей",
                "language": "ru",
                "nameType": "official"
            },
            {
                "value": "55R6M Triumfator-M",
                "language": "en",
        ...
        "identifiers": [
            ...
            {
                "value": "Triumfator-M",
                "identifierType": "CODE_NAME",
                "issuingAuthority": "Almaz-Antey"
            },
            {
                "value": "SA-X-NN",
        ...
        "vehicleType": "missile-defense-system",
        "make": "Almaz-Antey Air Defence Concern",
        "model": "S-500 Prometheus",
        "year": 2021,
        ...
        "representations": [
            {
                "id": "s500-wiki",
                "type": "webpage",
                "title": "S-500 missile system - Wikipedia",
                "description": "Wikipedia article about the S-500 Prometheus missile defense system",
                "url": "https://en.wikipedia.org/wiki/S-500_missile_system"
            }
        ],
        "temporalParts": [
            {
                "id": "s500-development-start",
                "stateType": "development",
                "startTime": "2010-01-01T00:00:00Z",
                "location": "Russia"
            },
            {
                "id": "s500-production-announcement",
                "stateType": "production_announcement",
                "startTime": "2019-06-30T00:00:00Z",
                "location": "Russia"
            },
            {
                "id": "s500-first-service",
                "stateType": "commissioned",
                "startTime": "2021-10-13T00:00:00Z",
                "location": "Moscow Oblast"
            },
            {
                "id": `s500-deployed-${OPERATION_CONFIG.database.toLowerCase()}`,
        ...
        "states": [
            {
                "id": `s500-operational-${OPERATION_CONFIG.database.toLowerCase()}`,
                "stateType": "active_deployment",
                "startTime": "2024-06-01T00:00:00Z",
                "location": OPERATION_CONFIG.displayName
            }
        ],
        ...
        "specifications": {
            "systemType": "Surface-to-air missile/Anti-ballistic missile system",
            "manufacturer": "Almaz-Antey Air Defence Concern",
            "operationalRange": {
                "air_defense": "500 km",
                "anti_ballistic": "600 km"
            },
            "maxAltitude": "200 km",
            "maxSpeed": "7 km/s",
            "responseTime": "<4 seconds",
            "simultaneousTargets": 10,
            "primaryRole": "Anti-ballistic missile defense",
            "missiles": [
                "40N6M (anti-aircraft)",
                "77N6/77N6-N1 (anti-ballistic/anti-satellite)"
            ],
            "components": [
                "77P6 launch vehicle (BAZ-69096 10x10)",
                "55K6MA/85Zh6-2 command posts (BAZ-69092-12 6x6)",
                "91N6A(M) acquisition radar (Big Bird)",
                "96L6-TsP acquisition radar (Cheese Board)",
                "76T6 multimode engagement radar",
                "77T6 ABM engagement radar"
            ],
            "estimatedCost": "$700-800 million per system (2020)",
            "deploymentRegion": OPERATION_CONFIG.displayName
        }
    };
    
    // Create vehicle type definition for missile defense systems
    const missileDefenseSystemType = {
        "id": "missile-defense-system",
        "name": "Missile Defense System",
        "category": "military equipment",
        "subcategory": "air defense",
        "description": "Surface-to-air and anti-ballistic missile defense system",
        "characteristics": [
            {
                "name": "primary_role",
                "value": "missile defense",
                "dataType": "string"
            },
            {
                "name": "system_class",
    ...
    // Check if S-500 already exists (check for any S-500 in this database)
    const existingSystem = data.vehicles.find(v => 
        v.id.includes('weapon-system-s500-prometheus') || 
        (v.names && v.names.some(name => 
            name.value === 'S-500 Prometheus' || 
            name.value === 'С-500 Прометей' ||
            name.value === '55R6M Triumfator-M'
        ))
    );
    
    if (existingSystem) {
        console.log(`ℹ️ S-500 Prometheus already exists in ${OPERATION_CONFIG.displayName} database. Updating...`);
        const index = data.vehicles.findIndex(v => v.id === existingSystem.id);
        data.vehicles[index] = { ...s500MissileSystem, id: existingSystem.id };
    } else {
        // Add the system to the vehicles array
        data.vehicles.push(s500MissileSystem);
    }
    
    // Add the vehicle type if it doesn't already exist
    const existingVehicleType = data.vehicleTypes.find(vt => vt.id === "missile-defense-system");
    if (!existingVehicleType) {
        data.vehicleTypes.push(missileDefenseSystemType);
    }
    
    // Write the updated data back to the file
    fs.writeFileSync(filePath, JSON.stringify(data, null, 2));
    
    console.log(`✅ Successfully added S-500 Prometheus missile system to ${OPERATION_CONFIG.displayName}`);
    console.log('📁 File location:', filePath);
    console.log('🆔 Added system ID:', s500MissileSystem.id);
    console.log('🏷️ Vehicle type added:', missileDefenseSystemType.id);
    
    return data;
}

// Function to remove S-500 missile system from the JSON data
function removeS500MissileSystem() {
    const filePath = path.join('C:', 'ies4-military-database-analysis', 'data', OPERATION_CONFIG.dataFile);
    ...
    try {
        ...
        // Remove S-500 from vehicles array
        if (data.vehicles && Array.isArray(data.vehicles)) {
            const initialLength = data.vehicles.length;
            data.vehicles = data.vehicles.filter(vehicle => {
                // Check multiple possible identifiers for the S-500
                const isS500System = 
                    vehicle.id.includes('weapon-system-s500-prometheus') ||
                    ...
                    (vehicle.identifiers && vehicle.identifiers.some(id => 
                        id.value === '55R6M' || 
                        id.value === 'Triumfator-M' ||
                        id.value === 'SA-X-NN'
                    )) ||
                    (vehicle.make === 'Almaz-Antey Air Defence Concern' && 
                     vehicle.model === 'S-500 Prometheus');
        ...
        // Check if we should remove the missile defense system type
        // (only if no other missile defense systems remain)
        if (data.vehicleTypes && Array.isArray(data.vehicleTypes)) {
            const hasOtherMissileDefenseSystems = data.vehicles && data.vehicles.some(vehicle => 
                vehicle.vehicleType === 'missile-defense-system'
            );
            
            if (!hasOtherMissileDefenseSystems) {
                const initialTypeLength = data.vehicleTypes.length;
                data.vehicleTypes = data.vehicleTypes.filter(vt => 
                    vt.id !== 'missile-defense-system'
                );
        ...
        // Report results
        if (removedCount > 0) {
            console.log(`✅ Successfully removed ${removedCount} S-500 Prometheus record(s) from ${OPERATION_CONFIG.displayName}`);
            if (removedVehicleType) {
                console.log('✅ Also removed missile-defense-system vehicle type (no other missile defense systems found)');
            }
        } else {
            console.log(`ℹ️ No S-500 Prometheus records found to remove from ${OPERATION_CONFIG.displayName}`);
        }
        ...
        if (error.code === 'ENOENT') {
            ...
            console.error('❌ Error removing S-500 Prometheus:', error.message);
        }
        return false;
    }
}
...
function verifyFileStructure() {
    ...
    try {
        ...
        // Check for any S-500 references
        let s500Refs = 0;
        if (data.vehicles) {
            data.vehicles.forEach(vehicle => {
                if (vehicle.names && vehicle.names.some(name => 
                    name.value.toLowerCase().includes('s-500') ||
                    name.value.toLowerCase().includes('prometheus') ||
                    name.value.toLowerCase().includes('triumfator') ||
                    name.value.toLowerCase().includes('прометей')
                )) {
                    s500Refs++;
                    console.log(`   Found S-500 reference: ${vehicle.id}`);
                }
                
                if (vehicle.make === 'Almaz-Antey Air Defence Concern' && 
                    vehicle.model && vehicle.model.toLowerCase().includes('s-500')) {
                    s500Refs++;
                    console.log(`   Found S-500 reference: ${vehicle.id}`);
                }
            });
        ...
        console.log(`   S-500 references found: ${s500Refs}`);
        return true;
...
async function performAddOperation() {
    console.log(`🚀 Adding S-500 Prometheus to ${OPERATION_CONFIG.displayName} database...\n`);
    ...
    try {
        ...
        if (refreshSuccess) {
            ...
            console.log(`   ✅ S-500 Prometheus added to ${OPERATION_CONFIG.displayName}`);
...
async function performRemoveOperation() {
    console.log(`🚀 Removing S-500 Prometheus from ${OPERATION_CONFIG.displayName} database...\n`);
    ...
    try {
        ...
        success = removeS500MissileSystem();
        ...
        if (refreshSuccess) {
            ...
            console.log(`   ✅ S-500 Prometheus removed from ${OPERATION_CONFIG.displayName}`);
            console.log('   ✅ Database reloaded in web interface');
            console.log('   ✅ Analysis refreshed with updated data');
            console.log('   ✅ Cache cleared');
            console.log('\n🌐 You can now view the updated data in your web browser');
            console.log('💡 The S-500 Prometheus should no longer appear in any analysis');
        } else {
            console.log('\n⚠️ Partial success:');
            console.log(`   ✅ S-500 Prometheus removed from ${OPERATION_CONFIG.displayName}`);
            console.log('   ✅ Database reloaded in web interface');
...
function showUsage() {
    console.log('S-500 Prometheus Management Script (Enhanced)');
    console.log('============================================\n');
    console.log('Usage:');
    console.log('  node s500_management.js --add [--db DATABASE]    Add S-500 Prometheus to the database');
    console.log('  node s500_management.js --del [--db DATABASE]    Remove S-500 Prometheus from the database');
    console.log('  node s500_management.js --list                   List available databases');
    console.log('  node s500_management.js --help                   Show this help message\n');
    console.log('Parameters:');
    console.log('  --db DATABASE    Specify target database (default: OP7)');
    console.log('                   Examples: OP1, OP2, OP3, OP4, OP5, OP6, OP7\n');
    console.log('Examples:');
    console.log('  node s500_management.js --add                    # Add to OP7 (Odesa Oblast)');
    console.log('  node s500_management.js --add --db OP1           # Add to OP1 (Kyiv Oblast)');
    console.log('  node s500_management.js --del --db OP3           # Remove from OP3 (Dnipro Oblast)');
    console.log('  node s500_management.js --list                   # Show all databases\n');
    console.log('Description:');
    console.log('  This script manages the S-500 Prometheus missile defense system across');
    console.log('  multiple regional databases. It can add or remove the system from any');
    console.log('  specified database and automatically update the web interface if running.\n');
    console.log('System Details:');
    console.log('  - Next-generation surface-to-air and anti-ballistic missile system');
    console.log('  - 600 km range, 200 km altitude capability');
    console.log('  - Anti-satellite and anti-ballistic missile capabilities');
    console.log('  - Response time: <4 seconds\n');
