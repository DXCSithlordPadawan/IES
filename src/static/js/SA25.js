...
function displayHelp() {
    console.log('🚀 9K333 Verba (SA-25) Surface-to-Air Missile System Database Management Tool');
    console.log('===========================================================================\n');
    console.log('Usage: node SA25.js [command] [options]\n');
    console.log('Commands:');
    console.log('  --add              Add 9K333 Verba SAM system to specified database');
    console.log('  --del              Remove 9K333 Verba SAM system from specified database');
    ...
    console.log('  node SA25.js --add --db OP7     Add 9K333 Verba to Odesa Oblast database');
    console.log('  node SA25.js --del --db OP1     Remove 9K333 Verba from Donetsk Oblast database');
    console.log('  node SA25.js --list             Show all available databases');
    console.log('  node SA25.js --diagnostic       Run system diagnostics');
}

// Function to run comprehensive diagnostics
function runDiagnostics() {
    console.log('🔧 Running SA25.js Diagnostic Checks');
    console.log('===================================\n');
...
// Function to clear any cached data that might contain 9K333 Verba
...
// Function to add 9K333 Verba SAM system to the JSON data
function addVerba9K333SAM() {
    ...
    const missileSystemId = `missile-system-9k333-verba-${OPERATION_CONFIG.database.toLowerCase()}-001`;
    
    // Create the 9K333 Verba SAM entry following IES4 schema
    const verba9K333SAM = {
        "id": missileSystemId,
        "uri": "https://en.wikipedia.org/wiki/9K333_Verba",
        "type": "MissileSystem",
        "names": [
            {
                "value": "9K333 Verba",
        ...
        "missileSystemType": "portable-surface-to-air-missile",
        ...
        "representations": [
            {
                "id": "verba-wiki",
                "type": "webpage",
                "title": "9K333 Verba - Wikipedia",
                "description": "Wikipedia article about the 9K333 Verba portable surface-to-air missile system",
                "url": "https://en.wikipedia.org/wiki/9K333_Verba"
            }
        ],
        "temporalParts": [
            {
                "id": "verba-development-start",
                "stateType": "development_initiated",
                "startTime": "2007-01-01T00:00:00Z",
                "location": "Tula, Russia"
            },
            {
                "id": "verba-testing-phase",
                "stateType": "testing_phase",
                "startTime": "2012-01-01T00:00:00Z",
                "location": "Various Russian test sites"
            },
            {
                "id": "verba-service-entry",
                "stateType": "entered_service",
                "startTime": "2014-01-01T00:00:00Z",
                "location": "Russian Armed Forces"
            },
            {
                "id": `verba-deployed-${OPERATION_CONFIG.database.toLowerCase()}`,
        ...
        "states": [
            {
                "id": `verba-operational-${OPERATION_CONFIG.database.toLowerCase()}`,
        ...
        "specifications": {
            "length": "1.65 m",
            "diameter": "72 mm",
            "weight": "17 kg (complete system)",
            "missileWeight": "7.5 kg",
            "range": "100 m - 6,500 m",
            "altitude": "10 m - 4,500 m",
            "guidance": "Infrared homing",
            "warhead": "High explosive fragmentation",
            "warheadWeight": "1.47 kg",
            "crew": 1,
            "classification": "Man-portable air-defense system (MANPADS)",
            "launcher": "9P516",
            "seekerType": "Imaging infrared seeker",
            "maxSpeed": "Mach 2.5",
    ...
    // Create missile system type definition for portable SAMs
    const portableSAMType = {
        "id": "portable-surface-to-air-missile",
        "name": "Portable Surface-to-Air Missile",
        "category": "air defense system",
        "subcategory": "man-portable air defense",
        "description": "Shoulder-fired surface-to-air missile system for low-altitude air defense",
        "characteristics": [
            {
                "name": "primary_role",
                "value": "air defense",
    ...
    // Check if 9K333 Verba already exists (check for any Verba in this database)
    const existingMissileSystem = data.missileSystems.find(ms => 
        ms.id.includes('missile-system-9k333-verba') || 
        (ms.names && ms.names.some(name => 
            name.value === '9K333 Verba' || 
            name.value === '9К333 «Верба»' ||
            name.value === 'SA-25 Verba' ||
            name.value === 'Verba'
        ))
    );
    
    if (existingMissileSystem) {
        console.log(`ℹ️ 9K333 Verba SAM system already exists in ${OPERATION_CONFIG.displayName} database. Updating...`);
        const index = data.missileSystems.findIndex(ms => ms.id === existingMissileSystem.id);
        data.missileSystems[index] = { ...verba9K333SAM, id: existingMissileSystem.id };
    } else {
        // Add the missile system to the missile systems array
        data.missileSystems.push(verba9K333SAM);
    }
    
    // Add the missile system type if it doesn't already exist
    const existingMissileSystemType = data.missileSystemTypes.find(mst => mst.id === "portable-surface-to-air-missile");
    if (!existingMissileSystemType) {
        data.missileSystemTypes.push(portableSAMType);
    }
    
    // Write the updated data back to the file
    fs.writeFileSync(filePath, JSON.stringify(data, null, 2));
    
    console.log(`✅ Successfully added 9K333 Verba SAM system to ${OPERATION_CONFIG.displayName}`);
    console.log('📁 File location:', filePath);
    console.log('🆔 Added missile system ID:', verba9K333SAM.id);
    console.log('🏷️ Missile system type added:', portableSAMType.id);
    
    return data;
}

// Function to remove 9K333 Verba SAM system from the JSON data
function removeVerba9K333SAM() {
    const filePath = path.join('C:', 'ies4-military-database-analysis', 'data', OPERATION_CONFIG.dataFile);
    ...
    try {
        ...
        // Remove 9K333 Verba from missile systems array
        if (data.missileSystems && Array.isArray(data.missileSystems)) {
            ...
            data.missileSystems = data.missileSystems.filter(missileSystem => {
                // Check multiple possible identifiers for the 9K333 Verba
                const isVerba9K333 = 
                    missileSystem.id.includes('missile-system-9k333-verba') ||
        ...
        // Check if we should remove the portable SAM type
        // (only if no other portable SAMs remain)
        if (data.missileSystemTypes && Array.isArray(data.missileSystemTypes)) {
            const hasOtherPortableSAMs = data.missileSystems && data.missileSystems.some(missileSystem => 
                missileSystem.missileSystemType === 'portable-surface-to-air-missile'
            );
            
            if (!hasOtherPortableSAMs) {
                const initialTypeLength = data.missileSystemTypes.length;
                data.missileSystemTypes = data.missileSystemTypes.filter(mst => 
                    mst.id !== 'portable-surface-to-air-missile'
                );
        ...
        // Report results
        if (removedCount > 0) {
            console.log(`✅ Successfully removed ${removedCount} 9K333 Verba SAM system record(s) from ${OPERATION_CONFIG.displayName}`);
            if (removedMissileSystemType) {
                console.log('✅ Also removed portable-surface-to-air-missile system type (no other portable SAMs found)');
            }
        } else {
            console.log(`ℹ️ No 9K333 Verba SAM system records found to remove from ${OPERATION_CONFIG.displayName}`);
        }
        ...
        if (error.code === 'ENOENT') {
            ...
            console.error('❌ Error removing 9K333 Verba SAM system:', error.message);
        }
        return false;
    }
}
...
function verifyFileStructure() {
    ...
    try {
        ...
        // Check for any remaining 9K333 Verba references
        let remainingRefs = 0;
        if (data.missileSystems) {
            data.missileSystems.forEach(missileSystem => {
                if (missileSystem.names && missileSystem.names.some(name => 
                    name.value.toLowerCase().includes('9k333') || 
                    name.value.toLowerCase().includes('verba') ||
                    name.value.toLowerCase().includes('sa-25')
                )) {
                    remainingRefs++;
                    console.log(`   Found 9K333 Verba reference: ${missileSystem.id}`);
                }
            });
        }
        
        console.log(`   9K333 Verba references found: ${remainingRefs}`);
        
        if (remainingRefs === 0) {
            console.log('✅ No remaining 9K333 Verba SAM system references found');
        }
...
async function performAddOperation() {
    console.log(`🚀 Adding 9K333 Verba SAM system to ${OPERATION_CONFIG.displayName} database...\n`);
    ...
    try {
        ...
        if (refreshSuccess) {
            ...
            console.log(`   ✅ 9K333 Verba SAM system added to ${OPERATION_CONFIG.displayName}`);
...
async function performRemoveOperation() {
    console.log(`🚀 Removing 9K333 Verba SAM system from ${OPERATION_CONFIG.displayName} database...\n`);
    ...
    try {
        ...
        if (refreshSuccess) {
            ...
            console.log(`   ✅ 9K333 Verba SAM system removed from ${OPERATION_CONFIG.displayName}`);
            console.log('   ✅ Database reloaded in web interface');
            console.log('   ✅ Analysis refreshed with updated data');
            console.log('   ✅ Cache cleared');
            console.log('\n🌐 You can now view the updated data in your web browser');
            console.log('💡 The 9K333 Verba SAM system should no longer appear in any analysis');
        } else {
            console.log('\n⚠️ Partial success:');
            console.log(`   ✅ 9K333 Verba SAM system removed from ${OPERATION_CONFIG.displayName}`);
            console.log('   ✅ Database reloaded in web interface');
...
if (require.main === module) {
    ...
    try {
        ...
        try {
            ...
            console.log('\n💡 Try running: node SA25.js --diagnostic');
            process.exit(1);
        }
