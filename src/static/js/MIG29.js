...
function displayHelp() {
    console.log('✈️ Mikoyan MiG-29 Aircraft Database Management Tool');
    console.log('================================================\n');
    console.log('Usage: node MIG29.js [command] [options]\n');
    console.log('Commands:');
    console.log('  --add              Add MiG-29 aircraft to specified database');
    console.log('  --del              Remove MiG-29 aircraft from specified database');
    ...
    console.log('  node MIG29.js --add --db OP7     Add MiG-29 to Odesa Oblast database');
    console.log('  node MIG29.js --del --db OP1     Remove MiG-29 from Donetsk Oblast database');
    console.log('  node MIG29.js --list             Show all available databases');
    console.log('  node MIG29.js --diagnostic       Run system diagnostics');
}

// Function to run comprehensive diagnostics
function runDiagnostics() {
    console.log('🔧 Running MIG29.js Diagnostic Checks');
    console.log('====================================\n');
...
// Function to clear any cached data that might contain MiG-29
...
// Function to add MiG-29 aircraft to the JSON data
function addMig29Aircraft() {
    ...
    const aircraftId = `aircraft-mig29-fighter-${OPERATION_CONFIG.database.toLowerCase()}-001`;
    
    // Create the MiG-29 aircraft entry following IES4 schema
    const mig29Aircraft = {
        "id": aircraftId,
        "uri": "https://en.wikipedia.org/wiki/Mikoyan_MiG-29",
        "type": "Aircraft",
        "names": [
            {
                "value": "MiG-29",
        ...
        "aircraftType": "fourth-generation-fighter",
        "manufacturer": "Mikoyan",
        "model": "MiG-29",
        "firstFlight": 1977,
        "enteredService": 1983,
        "operator": "Various Air Forces",
        "location": OPERATION_CONFIG.displayName,
        "representations": [
            {
                "id": "mig29-wiki",
                "type": "webpage",
                "title": "Mikoyan MiG-29 - Wikipedia",
                "description": "Wikipedia article about the Mikoyan MiG-29 twin-engine jet fighter aircraft",
                "url": "https://en.wikipedia.org/wiki/Mikoyan_MiG-29"
            }
        ],
        "temporalParts": [
            {
                "id": "mig29-program-start",
                "stateType": "program_initiated",
                "startTime": "1974-01-01T00:00:00Z",
                "location": "Moscow, Russia"
            },
            {
                "id": "mig29-first-flight",
                "stateType": "first_flight",
                "startTime": "1977-10-06T00:00:00Z",
                "location": "Zhukovsky Airfield, Russia"
            },
            {
                "id": "mig29-production-start",
                "stateType": "production_started",
                "startTime": "1982-01-01T00:00:00Z",
                "location": "Multiple facilities, Soviet Union"
            },
            {
                "id": "mig29-service-entry",
                "stateType": "entered_service",
                "startTime": "1983-01-01T00:00:00Z",
                "location": "Soviet Air Force"
            },
            {
                "id": `mig29-deployed-${OPERATION_CONFIG.database.toLowerCase()}`,
        ...
        "states": [
            {
                "id": `mig29-operational-${OPERATION_CONFIG.database.toLowerCase()}`,
    ...
    // Create aircraft type definition for fourth-generation fighters
    const fourthGenFighterType = {
        "id": "fourth-generation-fighter",
        "name": "Fourth Generation Fighter",
        "category": "military aircraft",
        "subcategory": "fighter aircraft",
        "description": "Advanced multirole fighter aircraft with beyond-visual-range capability",
        "characteristics": [
            {
                "name": "primary_role",
    ...
    // Check if MiG-29 already exists (check for any MiG-29 in this database)
    const existingAircraft = data.aircraft.find(a => 
        a.id.includes('aircraft-mig29-fighter') || 
    ...
    if (existingAircraft) {
        console.log(`ℹ️ MiG-29 aircraft already exists in ${OPERATION_CONFIG.displayName} database. Updating...`);
        const index = data.aircraft.findIndex(a => a.id === existingAircraft.id);
        data.aircraft[index] = { ...mig29Aircraft, id: existingAircraft.id };
    } else {
        // Add the aircraft to the aircraft array
        data.aircraft.push(mig29Aircraft);
    }
    
    // Add the aircraft type if it doesn't already exist
    const existingAircraftType = data.aircraftTypes.find(at => at.id === "fourth-generation-fighter");
    if (!existingAircraftType) {
        data.aircraftTypes.push(fourthGenFighterType);
    }
    
    // Write the updated data back to the file
    fs.writeFileSync(filePath, JSON.stringify(data, null, 2));
    
    console.log(`✅ Successfully added MiG-29 aircraft to ${OPERATION_CONFIG.displayName}`);
    console.log('📁 File location:', filePath);
    console.log('🆔 Added aircraft ID:', mig29Aircraft.id);
    console.log('🏷️ Aircraft type added:', fourthGenFighterType.id);
    
    return data;
}

// Function to remove MiG-29 aircraft from the JSON data
function removeMig29Aircraft() {
    const filePath = path.join('C:', 'ies4-military-database-analysis', 'data', OPERATION_CONFIG.dataFile);
    ...
    try {
        ...
        // Remove MiG-29 from aircraft array
        if (data.aircraft && Array.isArray(data.aircraft)) {
            const initialLength = data.aircraft.length;
            data.aircraft = data.aircraft.filter(aircraft => {
                // Check multiple possible identifiers for the MiG-29
                const isMig29Aircraft = 
                    aircraft.id.includes('aircraft-mig29-fighter') ||
                    ...
                    (aircraft.identifiers && aircraft.identifiers.some(id => 
                        id.value === 'MiG-29' || 
                        id.value === 'Izdeliye 9-12' ||
                        id.value === 'Fulcrum'
                    ));
                
                return !isMig29Aircraft;
            });
            
            removedCount = initialLength - data.aircraft.length;
        }
        
        // Check if we should remove the fourth generation fighter type
        // (only if no other fourth generation fighters remain)
        if (data.aircraftTypes && Array.isArray(data.aircraftTypes)) {
            const hasOtherFourthGenFighters = data.aircraft && data.aircraft.some(aircraft => 
                aircraft.aircraftType === 'fourth-generation-fighter'
            );
            
            if (!hasOtherFourthGenFighters) {
                const initialTypeLength = data.aircraftTypes.length;
                data.aircraftTypes = data.aircraftTypes.filter(at => 
                    at.id !== 'fourth-generation-fighter'
                );
        ...
        // Report results
        if (removedCount > 0) {
            console.log(`✅ Successfully removed ${removedCount} MiG-29 aircraft record(s) from ${OPERATION_CONFIG.displayName}`);
            if (removedAircraftType) {
                console.log('✅ Also removed fourth-generation-fighter aircraft type (no other fourth gen fighters found)');
            }
        } else {
            console.log(`ℹ️ No MiG-29 aircraft records found to remove from ${OPERATION_CONFIG.displayName}`);
        }
        ...
        if (error.code === 'ENOENT') {
            ...
            console.error('❌ Error removing MiG-29 aircraft:', error.message);
        }
        return false;
    }
}
...
function verifyFileStructure() {
    ...
    try {
        ...
        // Check for any remaining MiG-29 references
        let remainingRefs = 0;
        if (data.aircraft) {
            data.aircraft.forEach(aircraft => {
                if (aircraft.names && aircraft.names.some(name => 
                    name.value.toLowerCase().includes('mig-29') || 
                    name.value.toLowerCase().includes('миг-29') ||
                    name.value.toLowerCase().includes('fulcrum')
                )) {
                    remainingRefs++;
                    console.log(`   Found MiG-29 reference: ${aircraft.id}`);
                }
            });
        }
        
        console.log(`   MiG-29 references found: ${remainingRefs}`);
        
        if (remainingRefs === 0) {
            console.log('✅ No remaining MiG-29 aircraft references found');
        }
...
async function performAddOperation() {
    console.log(`🚀 Adding MiG-29 aircraft to ${OPERATION_CONFIG.displayName} database...\n`);
    ...
    try {
        ...
        if (refreshSuccess) {
            ...
            console.log(`   ✅ MiG-29 aircraft added to ${OPERATION_CONFIG.displayName}`);
...
async function performRemoveOperation() {
    console.log(`🚀 Removing MiG-29 aircraft from ${OPERATION_CONFIG.displayName} database...\n`);
    ...
    try {
        ...
        if (refreshSuccess) {
            ...
            console.log(`   ✅ MiG-29 aircraft removed from ${OPERATION_CONFIG.displayName}`);
            console.log('   ✅ Database reloaded in web interface');
            console.log('   ✅ Analysis refreshed with updated data');
            console.log('   ✅ Cache cleared');
            console.log('\n🌐 You can now view the updated data in your web browser');
            console.log('💡 The MiG-29 aircraft should no longer appear in any analysis');
        } else {
            console.log('\n⚠️ Partial success:');
            console.log(`   ✅ MiG-29 aircraft removed from ${OPERATION_CONFIG.displayName}`);
            console.log('   ✅ Database reloaded in web interface');
...
if (require.main === module) {
    ...
    try {
        ...
        try {
            ...
            console.log('\n💡 Try running: node MIG29.js --diagnostic');
            process.exit(1);
        }
