...
function displayHelp() {
    console.log('🚀 SA-14 Gremlin Missile System Database Management Tool');
    console.log('======================================================\n');
    console.log('Usage: node SA14.js [command] [options]\n');
    console.log('Commands:');
    console.log('  --add              Add SA-14 Gremlin missile system to specified database');
    console.log('  --del              Remove SA-14 Gremlin missile system from specified database');
    ...
    console.log('  node SA14.js --add --db OP7     Add SA-14 to Odesa Oblast database');
    console.log('  node SA14.js --del --db OP1     Remove SA-14 from Donetsk Oblast database');
    console.log('  node SA14.js --list             Show all available databases');
    console.log('  node SA14.js --diagnostic       Run system diagnostics');
}

// Function to run comprehensive diagnostics
function runDiagnostics() {
    console.log('🔧 Running SA14.js Diagnostic Checks');
    console.log('===================================\n');
...
// Function to clear any cached data that might contain SA-14
...
// Function to add SA-14 Gremlin missile system to the JSON data
function addSA14MissileSystem() {
    ...
    const missileSystemId = `missile-system-sa14-gremlin-${OPERATION_CONFIG.database.toLowerCase()}-001`;
    
    // Create the SA-14 Gremlin missile system entry following IES4 schema
    const sa14MissileSystem = {
        "id": missileSystemId,
        "uri": "https://en.wikipedia.org/wiki/SA-14_Gremlin",
        "type": "MissileSystem",
        "names": [
            {
                "value": "SA-14 Gremlin",
        ...
        "missileSystemType": "man-portable-air-defense-system",
        "manufacturer": "KBM (Kolomna Design Bureau of Machine Building)",
        "model": "9K34 Strela-3",
        "firstDeployed": 1974,
        "enteredService": 1974,
        "operator": "Various (Soviet Union, Warsaw Pact, Export)",
        "location": OPERATION_CONFIG.displayName,
        "representations": [
            {
                "id": "sa14-wiki",
                "type": "webpage",
                "title": "SA-14 Gremlin - Wikipedia",
                "description": "Wikipedia article about the SA-14 Gremlin (9K34 Strela-3) man-portable air-defense system",
                "url": "https://en.wikipedia.org/wiki/SA-14_Gremlin"
            }
        ],
        "temporalParts": [
            {
                "id": "sa14-development-start",
            ...
            {
                "id": "sa14-service-entry",
                "stateType": "entered_service",
                "startTime": "1974-01-01T00:00:00Z",
                "location": "Soviet Armed Forces"
            },
            {
                "id": "sa14-production-start",
                "stateType": "production_started",
                "startTime": "1974-01-01T00:00:00Z",
                "location": "Kolomna, Soviet Union"
            },
            {
                "id": `sa14-deployed-${OPERATION_CONFIG.database.toLowerCase()}`,
        ...
        "states": [
            {
                "id": `sa14-operational-${OPERATION_CONFIG.database.toLowerCase()}`,
        ...
        "specifications": {
            ...
            "launcher": {
                "weight": "15.8 kg (complete system)",
                "length": "1.8 m",
                "crew": 1,
                "reloadTime": "10-15 seconds"
            },
            "performance": {
                "maxRange": "6 km",
                "minRange": "0.8 km",
                "maxAltitude": "4.5 km",
                "minAltitude": "0.01 km",
                "speed": "Mach 2.2 (approx. 650 m/s)",
                "guidance": "Infrared homing (all-aspect)",
                "warheadType": "HE fragmentation with laser proximity fuze",
                "killProbability": "0.3-0.6 (depending on target and conditions)"
            },
            "operationalEnvironment": {
                "temperatureRange": "-40°C to +60°C",
                "operatingAltitude": "Up to 4,500 m",
                "weatherCapability": "Limited in adverse weather",
                "deploymentTime": "< 30 seconds"
            },
            "classification": "Man-portable air-defense system (MANPADS)",
            "targetTypes": ["Aircraft", "Helicopters", "UAVs"],
            "deploymentRegion": OPERATION_CONFIG.displayName
        },
        "variants": [
            "9K34 Strela-3 (basic version)",
            "9K34M Strela-3M (improved version)",
            "Export variants with reduced capability"
        ],
        "capabilities": [
        ...
        "limitations": [
            "infrared countermeasures susceptibility",
            "limited range compared to vehicle-mounted systems",
            "weather dependency",
            "operator training requirements"
        ]
    };
    
    // Create missile system type definition for MANPADS
    const manpadsMissileType = {
        ...
        "description": "Shoulder-fired surface-to-air missile system designed for anti-aircraft defense",
        "characteristics": [
            {
                "name": "primary_role",
                "value": "air defense",
    ...
    // Check if SA-14 already exists (check for any SA-14 in this database)
    const existingMissileSystem = data.missileSystems.find(ms => 
        ms.id.includes('missile-system-sa14-gremlin') || 
        (ms.names && ms.names.some(name => 
            name.value === 'SA-14 Gremlin' || 
            name.value === '9K34 Strela-3' ||
            name.value === 'Strela-3' ||
            name.value.includes('SA-14')
        ))
    );
    
    if (existingMissileSystem) {
        console.log(`ℹ️ SA-14 Gremlin missile system already exists in ${OPERATION_CONFIG.displayName} database. Updating...`);
        const index = data.missileSystems.findIndex(ms => ms.id === existingMissileSystem.id);
        data.missileSystems[index] = { ...sa14MissileSystem, id: existingMissileSystem.id };
    } else {
        // Add the missile system to the missileSystems array
        data.missileSystems.push(sa14MissileSystem);
    }
    
    // Add the missile system type if it doesn't already exist
    const existingMissileSystemType = data.missileSystemTypes.find(mst => mst.id === "man-portable-air-defense-system");
    if (!existingMissileSystemType) {
        data.missileSystemTypes.push(manpadsMissileType);
    }
    
    // Write the updated data back to the file
    fs.writeFileSync(filePath, JSON.stringify(data, null, 2));
    
    console.log(`✅ Successfully added SA-14 Gremlin missile system to ${OPERATION_CONFIG.displayName}`);
    console.log('📁 File location:', filePath);
    console.log('🆔 Added missile system ID:', sa14MissileSystem.id);
    console.log('🏷️ Missile system type added:', manpadsMissileType.id);
    
    return data;
}

// Function to remove SA-14 Gremlin missile system from the JSON data
function removeSA14MissileSystem() {
    const filePath = path.join('C:', 'ies4-military-database-analysis', 'data', OPERATION_CONFIG.dataFile);
    ...
    try {
        ...
        // Remove SA-14 from missile systems array
        if (data.missileSystems && Array.isArray(data.missileSystems)) {
            ...
            data.missileSystems = data.missileSystems.filter(missileSystem => {
                // Check multiple possible identifiers for the SA-14
                const isSA14MissileSystem = 
                    missileSystem.id.includes('missile-system-sa14-gremlin') ||
                    (missileSystem.names && missileSystem.names.some(name => 
                        name.value === 'SA-14 Gremlin' || 
                        name.value === '9K34 Strela-3' ||
                        name.value === '9К34 Стрела-3' ||
                        name.value === 'Strela-3' ||
                        name.value.includes('SA-14')
                    )) ||
                ...
                return !isSA14MissileSystem;
            });
            
            removedCount = initialLength - data.missileSystems.length;
        }
        
        // Check if we should remove the MANPADS missile system type
        // (only if no other MANPADS remain)
        if (data.missileSystemTypes && Array.isArray(data.missileSystemTypes)) {
            const hasOtherMANPADS = data.missileSystems && data.missileSystems.some(missileSystem => 
                missileSystem.missileSystemType === 'man-portable-air-defense-system'
            );
            
            if (!hasOtherMANPADS) {
                const initialTypeLength = data.missileSystemTypes.length;
                data.missileSystemTypes = data.missileSystemTypes.filter(mst => 
                    mst.id !== 'man-portable-air-defense-system'
                );
        ...
        // Report results
        if (removedCount > 0) {
            console.log(`✅ Successfully removed ${removedCount} SA-14 Gremlin missile system record(s) from ${OPERATION_CONFIG.displayName}`);
            if (removedMissileSystemType) {
                console.log('✅ Also removed man-portable-air-defense-system type (no other MANPADS found)');
            }
        } else {
            console.log(`ℹ️ No SA-14 Gremlin missile system records found to remove from ${OPERATION_CONFIG.displayName}`);
        }
        ...
        if (error.code === 'ENOENT') {
            ...
            console.error('❌ Error removing SA-14 Gremlin missile system:', error.message);
        }
        return false;
    }
}
...
function verifyFileStructure() {
    ...
    try {
        ...
        // Check for any remaining SA-14 references
        let remainingRefs = 0;
        if (data.missileSystems) {
            data.missileSystems.forEach(missileSystem => {
                if (missileSystem.names && missileSystem.names.some(name => 
                    name.value.toLowerCase().includes('sa-14') || 
                    name.value.toLowerCase().includes('strela-3') ||
                    name.value.toLowerCase().includes('gremlin') ||
                    name.value.toLowerCase().includes('9k34')
                )) {
                    remainingRefs++;
                    console.log(`   Found SA-14 reference: ${missileSystem.id}`);
                }
            });
        }
        
        console.log(`   SA-14 references found: ${remainingRefs}`);
        
        if (remainingRefs === 0) {
            console.log('✅ No remaining SA-14 Gremlin missile system references found');
        }
...
async function performAddOperation() {
    console.log(`🚀 Adding SA-14 Gremlin missile system to ${OPERATION_CONFIG.displayName} database...\n`);
    ...
    try {
        ...
        if (refreshSuccess) {
            ...
            console.log(`   ✅ SA-14 Gremlin missile system added to ${OPERATION_CONFIG.displayName}`);
...
async function performRemoveOperation() {
    console.log(`🚀 Removing SA-14 Gremlin missile system from ${OPERATION_CONFIG.displayName} database...\n`);
    ...
    try {
        ...
        success = removeSA14MissileSystem();
        ...
        if (refreshSuccess) {
            ...
            console.log(`   ✅ SA-14 Gremlin missile system removed from ${OPERATION_CONFIG.displayName}`);
            console.log('   ✅ Database reloaded in web interface');
            console.log('   ✅ Analysis refreshed with updated data');
            console.log('   ✅ Cache cleared');
            console.log('\n🌐 You can now view the updated data in your web browser');
            console.log('💡 The SA-14 Gremlin missile system should no longer appear in any analysis');
        } else {
            console.log('\n⚠️ Partial success:');
            console.log(`   ✅ SA-14 Gremlin missile system removed from ${OPERATION_CONFIG.displayName}`);
            console.log('   ✅ Database reloaded in web interface');
...
if (require.main === module) {
    ...
    try {
        ...
        try {
            ...
            console.log('\n💡 Try running: node SA14.js --diagnostic');
            process.exit(1);
        }
