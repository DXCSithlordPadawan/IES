...
function displayHelp() {
    console.log('🚀 SA-15 Gauntlet Surface-to-Air Missile System Database Management Tool');
    console.log('===============================================\n');
    console.log('Usage: node SA15.js [command] [options]\n');
    console.log('Commands:');
    console.log('  --add              Add SA-15 SAM system to specified database');
    console.log('  --del              Remove SA-15 SAM system from specified database');
    ...
    console.log('  node SA15.js --add --db OP7     Add SA-15 to Odesa Oblast database');
    console.log('  node SA15.js --del --db OP1     Remove SA-15 from Donetsk Oblast database');
    console.log('  node SA15.js --list             Show all available databases');
    console.log('  node SA15.js --diagnostic       Run system diagnostics');
}

// Function to run comprehensive diagnostics
function runDiagnostics() {
    console.log('🔧 Running SA15.js Diagnostic Checks');
    console.log('====================================\n');
...
// Function to clear any cached data that might contain SA-15
...
// Function to add SA-15 Gauntlet SAM system to the JSON data
function addSa15System() {
    ...
    const systemId = `sam-sa15-gauntlet-${OPERATION_CONFIG.database.toLowerCase()}-001`;
    
    // Create the SA-15 Gauntlet SAM system entry following IES4 schema
    const sa15System = {
        "id": systemId,
        "uri": "https://en.wikipedia.org/wiki/SA-15_Gauntlet",
        "type": "SAMSystem",
        "names": [
            {
        ...
        "identifiers": [
            ...
            {
                "value": "9M330",
                "identifierType": "MISSILE_DESIGNATION",
                "issuingAuthority": "GRAU"
            },
            {
                "value": "9A331",
                "identifierType": "LAUNCHER_DESIGNATION",
                "issuingAuthority": "GRAU"
            }
        ],
        "systemType": "short-range-sam",
        "manufacturer": "Almaz-Antey",
        "model": "9K330 Tor",
        "firstDeployment": 1986,
        "enteredService": 1991,
        ...
        "representations": [
            {
                "id": "sa15-wiki",
                "type": "webpage",
                "title": "SA-15 Gauntlet - Wikipedia",
                "description": "Wikipedia article about the SA-15 Gauntlet surface-to-air missile system",
                "url": "https://en.wikipedia.org/wiki/SA-15_Gauntlet"
            }
        ],
        "temporalParts": [
            {
                "id": "sa15-development-start",
            ...
            {
                "id": "sa15-service-entry",
                "stateType": "entered_service",
                "startTime": "1991-01-01T00:00:00Z",
                "location": "Soviet Union"
            },
            {
                "id": `sa15-deployed-${OPERATION_CONFIG.database.toLowerCase()}`,
        ...
        "states": [
            {
                "id": `sa15-operational-${OPERATION_CONFIG.database.toLowerCase()}`,
                "stateType": "active_service",
                "startTime": "2014-01-01T00:00:00Z",
                "location": OPERATION_CONFIG.displayName
            }
        ],
        // Additional SAM system-specific properties
        "specifications": {
            "vehicleLength": "7.5 m",
            "vehicleWidth": "3.3 m",
            "vehicleHeight": "5.1 m (with radar)",
            "vehicleWeight": "34,000 kg",
            "crew": 3,
            "classification": "Short-range surface-to-air missile system",
            "missileType": "9M330/9M331",
            "missileWeight": "165-167 kg",
            "missileLength": "2.9 m",
            "missileDiameter": "235 mm",
            "warheadType": "HE-fragmentation",
            "warheadWeight": "14.5-15 kg",
            "maxRange": "12 km",
            "minRange": "1.5 km",
            "maxAltitude": "6,000 m",
            "minAltitude": "10 m",
            "missileSpeed": "850 m/s",
            "guidanceSystem": "Radio command guidance",
            "radarType": "3D phased array",
            "radarRange": "16-25 km",
            "reactionTime": "5-8 seconds",
            "deploymentRegion": OPERATION_CONFIG.displayName,
            "readyToFireTime": "3 minutes",
            "simultaneousTargets": "4",
            "missileCapacity": "8 missiles"
        },
        "variants": [
            "9K330 Tor (baseline)",
            "9K331 Tor-M1",
    ...
    // Create SAM system type definition for short-range systems
    const shortRangeSamType = {
        "id": "short-range-sam",
        "name": "Short Range Surface-to-Air Missile System",
        "category": "air defense system",
        "subcategory": "surface-to-air missile",
        "description": "Mobile short-range air defense system for low to medium altitude threats",
        "characteristics": [
            {
                "name": "primary_role",
                "value": "point air defense",
                "dataType": "string"
            },
            {
                "name": "crew_size",
    ...
    // Add SAM systems array if it doesn't exist
    if (!data.samSystems) {
        data.samSystems = [];
    }
    
    // Add SAM system types array if it doesn't exist
    if (!data.samSystemTypes) {
        data.samSystemTypes = [];
    }
    
    // Check if SA-15 already exists (check for any SA-15 in this database)
    const existingSystem = data.samSystems.find(s => 
        s.id.includes('sam-sa15-gauntlet') || 
        (s.names && s.names.some(name => 
            name.value === 'SA-15 Gauntlet' || 
            name.value === '9K330 Tor' ||
            name.value === '9К330 Тор'
        ))
    );
    
    if (existingSystem) {
        console.log(`ℹ️ SA-15 Gauntlet system already exists in ${OPERATION_CONFIG.displayName} database. Updating...`);
        const index = data.samSystems.findIndex(s => s.id === existingSystem.id);
        data.samSystems[index] = { ...sa15System, id: existingSystem.id };
    } else {
        // Add the system to the SAM systems array
        data.samSystems.push(sa15System);
    }
    
    // Add the SAM system type if it doesn't already exist
    const existingSystemType = data.samSystemTypes.find(st => st.id === "short-range-sam");
    if (!existingSystemType) {
        data.samSystemTypes.push(shortRangeSamType);
    }
    
    // Write the updated data back to the file
    fs.writeFileSync(filePath, JSON.stringify(data, null, 2));
    
    console.log(`✅ Successfully added SA-15 Gauntlet system to ${OPERATION_CONFIG.displayName}`);
    console.log('📁 File location:', filePath);
    console.log('🆔 Added system ID:', sa15System.id);
    console.log('🏷️ System type added:', shortRangeSamType.id);
    
    return data;
}

// Function to remove SA-15 Gauntlet system from the JSON data
function removeSa15System() {
    const filePath = path.join('C:', 'ies4-military-database-analysis', 'data', OPERATION_CONFIG.dataFile);
    ...
    try {
        ...
        // Remove SA-15 from SAM systems array
        if (data.samSystems && Array.isArray(data.samSystems)) {
            const initialLength = data.samSystems.length;
            data.samSystems = data.samSystems.filter(system => {
                // Check multiple possible identifiers for the SA-15
                const isSa15System = 
                    system.id.includes('sam-sa15-gauntlet') ||
            ...
            removedCount = initialLength - data.samSystems.length;
        }
        
        // Check if we should remove the short range SAM type
        // (only if no other short range SAM systems remain)
        if (data.samSystemTypes && Array.isArray(data.samSystemTypes)) {
            const hasOtherShortRangeSams = data.samSystems && data.samSystems.some(system => 
                system.systemType === 'short-range-sam'
            );
            
            if (!hasOtherShortRangeSams) {
                const initialTypeLength = data.samSystemTypes.length;
                data.samSystemTypes = data.samSystemTypes.filter(st => 
                    st.id !== 'short-range-sam'
                );
                removedSystemType = initialTypeLength > data.samSystemTypes.length;
            }
        }
        ...
        // Report results
        if (removedCount > 0) {
            console.log(`✅ Successfully removed ${removedCount} SA-15 Gauntlet system record(s) from ${OPERATION_CONFIG.displayName}`);
            if (removedSystemType) {
                console.log('✅ Also removed short-range-sam system type (no other short range SAMs found)');
            }
        } else {
            console.log(`ℹ️ No SA-15 Gauntlet system records found to remove from ${OPERATION_CONFIG.displayName}`);
        }
        ...
        if (error.code === 'ENOENT') {
            ...
            console.error('❌ Error removing SA-15 Gauntlet system:', error.message);
        }
        return false;
    }
}
...
function verifyFileStructure() {
    ...
    try {
        ...
        console.log(`   SAM Systems: ${data.samSystems ? data.samSystems.length : 0}`);
        console.log(`   SAM System Types: ${data.samSystemTypes ? data.samSystemTypes.length : 0}`);
        
        // Check for any remaining SA-15 references
        let remainingRefs = 0;
        if (data.samSystems) {
            data.samSystems.forEach(system => {
                if (system.names && system.names.some(name => 
                    name.value.toLowerCase().includes('sa-15') || 
                    name.value.toLowerCase().includes('gauntlet') ||
                    name.value.toLowerCase().includes('tor') ||
                    name.value.toLowerCase().includes('9k330')
                )) {
                    remainingRefs++;
                    console.log(`   Found SA-15 reference: ${system.id}`);
                }
            });
        }
        
        console.log(`   SA-15 references found: ${remainingRefs}`);
        
        if (remainingRefs === 0) {
            console.log('✅ No remaining SA-15 Gauntlet system references found');
        }
...
async function performAddOperation() {
    console.log(`🚀 Adding SA-15 Gauntlet system to ${OPERATION_CONFIG.displayName} database...\n`);
    ...
    try {
        ...
        if (refreshSuccess) {
            ...
            console.log(`   ✅ SA-15 Gauntlet system added to ${OPERATION_CONFIG.displayName}`);
...
async function performRemoveOperation() {
    console.log(`🚀 Removing SA-15 Gauntlet system from ${OPERATION_CONFIG.displayName} database...\n`);
    ...
    try {
        ...
        if (refreshSuccess) {
            ...
            console.log(`   ✅ SA-15 Gauntlet system removed from ${OPERATION_CONFIG.displayName}`);
            console.log('   ✅ Database reloaded in web interface');
            console.log('   ✅ Analysis refreshed with updated data');
            console.log('   ✅ Cache cleared');
            console.log('\n🌐 You can now view the updated data in your web browser');
            console.log('💡 The SA-15 Gauntlet system should no longer appear in any analysis');
        } else {
            console.log('\n⚠️ Partial success:');
            console.log(`   ✅ SA-15 Gauntlet system removed from ${OPERATION_CONFIG.displayName}`);
            console.log('   ✅ Database reloaded in web interface');
...
if (require.main === module) {
    ...
    try {
        ...
        try {
            ...
            console.log('\n💡 Try running: node SA15.js --diagnostic');
            process.exit(1);
        }
