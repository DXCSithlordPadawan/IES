...
// Function to clear any cached data that might contain BTR-90
...
// Function to add BTR-90 armored personnel carrier to the JSON data
function addBTR90() {
    ...
    const vehicleId = `vehicle-btr-90-${OPERATION_CONFIG.database.toLowerCase()}-001`;
    
    // Create the BTR-90 vehicle entry following IES4 schema
    const btr90 = {
        "id": vehicleId,
        "uri": "https://en.wikipedia.org/wiki/BTR-90",
        "type": "Vehicle",
        "names": [
            {
                "value": "BTR-90",
        ...
        "identifiers": [
            {
                ...
                "issuingAuthority": "GAZ"
            },
            {
                "value": "8x8 APC",
                "identifierType": "CONFIGURATION",
                "issuingAuthority": "Russian Army"
            }
        ],
        "vehicleType": "armored-personnel-carrier",
        "make": "Arzamas Machine Building Plant (AMZ)",
        "model": "BTR-90",
        "year": 1994,
        "owner": "Russian National Guard",
        "location": OPERATION_CONFIG.displayName,
        "representations": [
            {
                "id": "btr90-wiki",
                "type": "webpage",
                "title": "BTR-90 - Wikipedia",
                "description": "Wikipedia article about the BTR-90 armored personnel carrier",
                "url": "https://en.wikipedia.org/wiki/BTR-90"
            }
        ],
        "temporalParts": [
            {
                "id": "btr90-development-start",
                "stateType": "development",
                "startTime": "1993-01-01T00:00:00Z",
                "location": "Arzamas Machine Building Plant"
            },
            {
                "id": "btr90-first-public-display",
                "stateType": "first_display",
                "startTime": "1994-01-01T00:00:00Z",
                "location": "Russia"
            },
            {
                "id": "btr90-production-start",
                "stateType": "production",
                "startTime": "2008-05-01T00:00:00Z",
                "location": "Arzamas Machine Building Plant"
            },
            {
                "id": `btr90-deployed-${OPERATION_CONFIG.database.toLowerCase()}`,
        ...
        "states": [
            {
                "id": `btr90-operational-${OPERATION_CONFIG.database.toLowerCase()}`,
        ...
        "specifications": {
            ...
            "classification": "8x8 Armored Personnel Carrier",
            "crew": "3 (commander, gunner, driver)",
            "troops": "7 personnel",
            "armament": {
                "primary": "30mm Shipunov 2A42 autocannon",
                "secondary": [
                    "7.62mm PKT coaxial machine gun",
                    "30mm AGS-17D automatic grenade launcher",
                    "9M113 Konkurs ATGM system"
                ]
            },
            "ammunition": {
                "30mm": "500 rounds",
            ...
            "engine": "Turbocharged multi-fuel diesel, 510 hp",
            "mobility": {
                "max_speed_road": "90 km/h",
                "max_speed_water": "9 km/h",
                "range": "800 km"
            },
            "protection": "Welded steel armor, protection against 14.5mm rounds over frontal arc",
            "deploymentRegion": OPERATION_CONFIG.displayName
        },
        "variants": [
            "BTR-90M",
            "BTR-90K (command variant)",
            "BTR-90A (artillery observation)",
            "BREM-90 (recovery variant)"
        ]
    };
    
    // Create vehicle type definition for armored personnel carriers
    const apcVehicleType = {
        "id": "armored-personnel-carrier",
        "name": "Armored Personnel Carrier",
        "category": "combat vehicle",
        "subcategory": "armored personnel carrier",
        "description": "8x8 wheeled armored personnel carrier with fire support capability",
        "characteristics": [
            {
                "name": "primary_role",
                "value": "personnel transport and fire support",
                "dataType": "string"
            },
            {
                "name": "vehicle_class",
    ...
    // Check if BTR-90 already exists (check for any BTR-90 in this database)
    const existingVehicle = data.vehicles.find(v => 
        v.id.includes('vehicle-btr-90') || 
    ...
    if (existingVehicle) {
        console.log(`ℹ️ BTR-90 already exists in ${OPERATION_CONFIG.displayName} database. Updating...`);
        const index = data.vehicles.findIndex(v => v.id === existingVehicle.id);
        data.vehicles[index] = { ...btr90, id: existingVehicle.id };
    } else {
        // Add the vehicle to the vehicles array
        data.vehicles.push(btr90);
    }
    
    // Add the vehicle type if it doesn't already exist
    const existingVehicleType = data.vehicleTypes.find(vt => vt.id === "armored-personnel-carrier");
    if (!existingVehicleType) {
        data.vehicleTypes.push(apcVehicleType);
    }
    
    // Write the updated data back to the file
    fs.writeFileSync(filePath, JSON.stringify(data, null, 2));
    
    console.log(`✅ Successfully added BTR-90 armored personnel carrier to ${OPERATION_CONFIG.displayName}`);
    console.log('📁 File location:', filePath);
    console.log('🆔 Added vehicle ID:', btr90.id);
    console.log('🏷️ Vehicle type added:', apcVehicleType.id);
    
    return data;
}

// Function to remove BTR-90 armored personnel carrier from the JSON data
function removeBTR90() {
    const filePath = path.join('C:', 'ies4-military-database-analysis', 'data', OPERATION_CONFIG.dataFile);
    ...
    try {
        ...
        // Remove BTR-90 from vehicles array
        if (data.vehicles && Array.isArray(data.vehicles)) {
            const initialLength = data.vehicles.length;
            data.vehicles = data.vehicles.filter(vehicle => {
                // Check multiple possible identifiers for the BTR-90
                const isBTR90 = 
                    vehicle.id.includes('vehicle-btr-90') ||
                    ...
                    (vehicle.make && vehicle.make.includes('Arzamas Machine Building Plant')) ||
                    (vehicle.model && vehicle.model === 'BTR-90');
        ...
        // Check if we should remove the armored personnel carrier type
        // (only if no other APCs remain)
        if (data.vehicleTypes && Array.isArray(data.vehicleTypes)) {
            const hasOtherAPCs = data.vehicles && data.vehicles.some(vehicle => 
                vehicle.vehicleType === 'armored-personnel-carrier'
            );
            
            if (!hasOtherAPCs) {
                const initialTypeLength = data.vehicleTypes.length;
                data.vehicleTypes = data.vehicleTypes.filter(vt => 
                    vt.id !== 'armored-personnel-carrier'
                );
        ...
        // Report results
        if (removedCount > 0) {
            console.log(`✅ Successfully removed ${removedCount} BTR-90 record(s) from ${OPERATION_CONFIG.displayName}`);
            if (removedVehicleType) {
                console.log('✅ Also removed armored-personnel-carrier vehicle type (no other APCs found)');
            }
        } else {
            console.log(`ℹ️ No BTR-90 records found to remove from ${OPERATION_CONFIG.displayName}`);
        }
        ...
        if (error.code === 'ENOENT') {
            ...
            console.error('❌ Error removing BTR-90:', error.message);
        }
        return false;
    }
}
...
function verifyFileStructure() {
    ...
    try {
        ...
        // Check for any BTR-90 references
        let btr90Refs = 0;
        if (data.vehicles) {
            data.vehicles.forEach(vehicle => {
                if (vehicle.names && vehicle.names.some(name => 
                    name.value.toLowerCase().includes('btr-90') ||
                    name.value.toLowerCase().includes('gaz-5923') ||
                    name.value.toLowerCase().includes('бтр-90')
                )) {
                    btr90Refs++;
                    console.log(`   Found BTR-90 reference: ${vehicle.id}`);
                }
                
                if ((vehicle.make && vehicle.make.includes('Arzamas Machine Building Plant')) && 
                    vehicle.model && vehicle.model.toLowerCase().includes('btr-90')) {
                    btr90Refs++;
                    console.log(`   Found BTR-90 reference: ${vehicle.id}`);
                }
            });
        ...
        console.log(`   BTR-90 references found: ${btr90Refs}`);
        return true;
...
async function performAddOperation() {
    console.log(`🚗 Adding BTR-90 to ${OPERATION_CONFIG.displayName} database...\n`);
    ...
    try {
        ...
        if (refreshSuccess) {
            ...
            console.log(`   ✅ BTR-90 added to ${OPERATION_CONFIG.displayName}`);
...
async function performRemoveOperation() {
    console.log(`🚗 Removing BTR-90 from ${OPERATION_CONFIG.displayName} database...\n`);
    ...
    try {
        ...
        if (refreshSuccess) {
            ...
            console.log(`   ✅ BTR-90 removed from ${OPERATION_CONFIG.displayName}`);
            console.log('   ✅ Database reloaded in web interface');
            console.log('   ✅ Analysis refreshed with updated data');
            console.log('   ✅ Cache cleared');
            console.log('\n🌐 You can now view the updated data in your web browser');
            console.log('💡 The BTR-90 should no longer appear in any analysis');
        } else {
            console.log('\n⚠️ Partial success:');
            console.log(`   ✅ BTR-90 removed from ${OPERATION_CONFIG.displayName}`);
            console.log('   ✅ Database reloaded in web interface');
...
function showUsage() {
    console.log('BTR-90 Management Script (Enhanced)');
    console.log('===================================\n');
    console.log('Usage:');
    console.log('  node btr90_management.js --add [--db DATABASE]    Add BTR-90 to the database');
    console.log('  node btr90_management.js --del [--db DATABASE]    Remove BTR-90 from the database');
    console.log('  node btr90_management.js --list                   List available databases');
    console.log('  node btr90_management.js --help                   Show this help message\n');
    console.log('Parameters:');
    console.log('  --db DATABASE    Specify target database (default: OP7)');
    console.log('                   Examples: OP1, OP2, OP3, OP4, OP5, OP6, OP7\n');
    console.log('Examples:');
    console.log('  node btr90_management.js --add                    # Add to OP7 (Odesa Oblast)');
    console.log('  node btr90_management.js --add --db OP1           # Add to OP1 (Kyiv Oblast)');
    console.log('  node btr90_management.js --del --db OP3           # Remove from OP3 (Dnipro Oblast)');
    console.log('  node btr90_management.js --list                   # Show all databases\n');
    console.log('Description:');
    console.log('  This script manages the BTR-90 armored personnel carrier across multiple');
    console.log('  regional databases. It can add or remove the vehicle from any specified');
    console.log('  database and automatically update the web interface if running.\n');
    console.log('Vehicle Details:');
    console.log('  - 8x8 wheeled armored personnel carrier');
    console.log('  - Combat weight: 20,900 kg, Crew: 3 + 7 troops');
    console.log('  - Main armament: 30mm Shipunov 2A42 autocannon');
    console.log('  - Secondary: ATGM system, grenade launcher');
    console.log('  - Engine: Turbocharged multi-fuel diesel, 510 hp\n');
